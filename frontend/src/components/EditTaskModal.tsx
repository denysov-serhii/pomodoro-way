...
const EditTaskModal: React.FC<EditTaskModalProps> = ({ visible, onClose, task }) => {
  ...
  if (!context) {
    throw new Error('EditTaskModal must be used within AppProvider');
  }
  const { updateTask, projects, folders, tags } = context;
  ...
  const [selectedFolder, setSelectedFolder] = useState<string | null>(task.folderId || null);
  ...
  useEffect(() => {
    ...
    setSelectedFolder(task.folderId || null);
